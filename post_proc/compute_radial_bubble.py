...
with hoomd.open(name=infile, mode='rb') as t:
    ...
    numA = [ 0. for i in range(0, nBins) ]
    numB = [ 0. for i in range(0, nBins) ]
    ...
    for j in range(start, end):
        ...
        for k in range(0, partNum):
            if ids[k] == lcID:
                lc_pos.append(pos[k])
                if typ[k] == 0:
                    lc_posA.append(pos[k])
                    lc_typ.append(0)
                else:
                    lc_posB.append(pos[k])
                    lc_typ.append(1)
                lc_ids.append(k)
...
for j in range(0, len(r_bins)):
    ...
    g.write('{0:.0f}'.format(numA[j]).center(25) + ' ')
    g.write('{0:.0f}'.format(numB[j]).center(25) + ' ')
