...
box_width = 2
while side % box_width != 0:                            # grid boxes of width 2 sigma
...
#    dp_density = 0
    ...
    dp_density /= dense_num                 # number in each bin
    dp_density /= (np.pi * (dist_min**2))   # divide by bin area
    a_clust = dense_num / dp_density        # area occupied by cluster
    ...
    f.write(('{0:.2f}'.format(tst*0.000001)).center(10) + ' ' +\
            ...
            str(dp_density).center(10) + ' ' +\
            str(gp_density).center(10) + '\n')
    f.close()
