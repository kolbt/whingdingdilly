...
for mmm in range(start, stop):
    ...
    for iii in range(0,part_num):
        ...
        # large value moves a tiny bit to the left
        if xs[iii] <= -(0.9 * (box_data[0] / 2.0)):
#        if xs[iii] <= -(0.85 * (box_data[0] / 2.0)):
#        if xs[iii] <= -(0.7 * (box_data[0] / 2.0)):
            xs[iii] += box_data[0]
        # large value moves a tiny bit down
        if ys[iii] <= -(0.55 * (box_data[0] / 2.0)):
#        if ys[iii] <= -(0.5 * (box_data[0] / 2.0)):
#        if ys[iii] <= -(0.45 * (box_data[0] / 2.0)):
            ys[iii] += box_data[0]
