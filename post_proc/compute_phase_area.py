...
Designations Explained:
      ...
      dense_total : total number of dense phase particles
          ...
          l_clust : largest individual cluster
           ...
           phiEff : effective area fraction from particle areas
          AlcArea : area of A particles in largest cluster
          BlcArea : area of B particles in largest cluster
           lcArea : area of all particles in largest cluster
       AdenseArea : area of A particles in all clusters
       BdenseArea : area of B particles in all clusters
        denseArea : area of all particles in all clusters
            a_box : area of simulation box
'''
...
f.write('Timestep'.center(10) + ' ' +\
        ...
        'Lc_area_B'.center(10) + ' ' +\
        'lc_area'.center(10) + ' ' +\
        'den_area_A'.center(10) + ' ' +\
        'den_area_B'.center(10) + ' ' +\
        'den_area'.center(10) + ' ' +\
        'Box_area'.center(10) + '\n')
f.close()
...
for j in range(start, end):
    ...
    lcArea = 0.0           # largest cluster area
    denseArea = 0.0           # total cluster area
    ...
    # I want to know the area of each phase...
    lcArea = 0.     # area of particles in largest cluster
    AlcArea = 0.    # area of A particles in largest cluster
    BlcArea = 0.    # area of B particles in largest cluster
    denseArea = 0.  # area of particles in dense phase
    AdenseArea = 0. # area of A particles in dense phase
    BdenseArea = 0. # area of B particles in dense phase
    gasArea = 0.    # area of particles in gas phase
    AgasArea = 0.   # area of A particles in gas phase
    ...
    for k in range(0, partNum):
        ...
        # This is in dense phase
            ...
            dense_num += 1
            if typ[k] == 0:
                AdenseArea += sigSquare
                dense_A += 1
            else:
                BdenseArea += sigSquare
                dense_B += 1

    # Compute some things...
    if clust_num != 0:
        mcs = dense_num / clust_num
    lc_num = lc_numA + lc_numB

    # Largest cluster areas by parts
    AlcArea *= con
    BlcArea *= con
    lcArea *= con
    # All cluster area by parts
    AdenseArea *= con
    BdenseArea *= con
    denseArea *= con
    # Gas area by parts
    AgasArea *= con
    BgasArea *= con
    pgA = gasArea * con         # area of particles in gas phase
    tgA = a_box - denseArea     # area of gas phase
    # Now get area fractions
    phiLC = lcArea / a_box
    phiC = denseArea / a_box
    phiG = tgA / a_box
    phiEff *= ( con / a_box )
    ...
    f.write(('{0:.2f}'.format(tst)).center(10) + ' ' +\
            ...
            '{:.2f}'.format(phiEff).center(10) + ' ' + \
            '{:.4f}'.format(AlcArea).center(10) + ' ' + \
            '{:.4f}'.format(BlcArea).center(10) + ' ' + \
            '{:.4f}'.format(lcArea).center(10) + ' ' + \
            '{:.4f}'.format(AdenseArea).center(10) + ' ' +\
            '{:.4f}'.format(BdenseArea).center(10) + ' ' +\
            '{:.4f}'.format(denseArea).center(10) + ' ' +\
            '{:.2f}'.format(a_box).center(10) + '\n')
    f.close()
