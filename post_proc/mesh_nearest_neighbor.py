...
ALL = np.zeros((end, part_num), dtype=np.int)
AA = np.zeros((end, part_num), dtype=np.int)
BB = np.zeros((end, part_num), dtype=np.int)
AB = np.zeros((end, part_num), dtype=np.int)
BA = np.zeros((end, part_num), dtype=np.int)
...
for iii in range(start, end):
        ...
        for h in range(0, len(h_list)):
            for v in range(0, len(v_list)):
                ...
                for b in range(0, len(binParts[h_list[h]][v_list[v]])):
                    ...
                    if 0.1 < r <= r_cut:
                        ALL[iii][jjj] += 1                      # Any ref
                        if typ[jjj] == 0 and typ[ref] == 0:     # AA neighbors
                            AA[iii][jjj] += 1
                        elif typ[jjj] == 1 and typ[ref] == 1:   # BB neighbors
                            BB[iii][jjj] += 1
                        elif typ[jjj] == 0 and typ[ref] == 1:   # AB neighbors
                            AB[iii][jjj] += 1
                        else:                                   # BA neighbors
                            BA[iii][jjj] += 1

    # Plot number of neighbors as histogram for each type
    xmin = 1
    xmax = 6
    # ymin = 0
    # ymax = 20
    ...
    fig, ax = plt.subplots()
    plt.hist(ALL[iii], bins=6, range=[0.5, 6.5], align='mid', normed=True)
    # plt.xlim(xmin, xmax)
    # plt.ylim(ymin, ymax)
    ax.set(xlabel='Number of nearest neighbors',
           ylabel='Percentage of particles')
    # plt.legend(loc='upper right')
    plt.savefig('ALL_neighbors_' + b_file + '_' + str(iii) + '.png', dpi=1000)
    plt.close()
    ...
    plt.hist(AA[iii], bins=6, range=[0.5, 6.5], align='mid', normed=True)
    # plt.xlim(xmin, xmax)
    # ax.set(xlabel='Number of nearest neighbors',
    #        ylabel='Percentage of particles')
    plt.title('AA')
    ...
    plt.hist(BB[iii], bins=6, range=[0.5, 6.5], align='mid', normed=True)
    # plt.xlim(xmin, xmax)
    plt.title('BB')

    ax = fig.add_subplot(223)
    plt.hist(AB[iii], bins=6, range=[0.5, 6.5], align='mid', normed=True)
    # plt.xlim(xmin, xmax)
    plt.title('AB')

    ax = fig.add_subplot(224)
    plt.hist(BA[iii], bins=6, range=[0.5, 6.5], align='mid', normed=True)
    # plt.xlim(xmin, xmax)
    plt.title('BA')

    # plt.legend(loc='upper right')
    plt.tight_layout()
    plt.savefig('multi_neighbors_' + b_file + '_' + str(iii) + '.png', dpi=1000)
    plt.close()
