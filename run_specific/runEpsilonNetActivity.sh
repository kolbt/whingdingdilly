...
if [ $answer == "y" ]; then
    ...
    template='/nas/longleaf/home/kolbt/whingdingdilly/run_specific/epsilonNetActivity.py'
    sedtype='sed'
    submit='sbatch'
else
    ...
    template='/Users/kolbt/Desktop/compiled/whingdingdilly/run_specific/epsilonNetActivity.py'
    sedtype='gsed'
    submit='sh'
fi
...
do
    ...
    echo "density, as a percent, is ${phi}, input new value or same value."
    read input
    if ! [[ -z "$input" ]]; then
        phi=$input
    fi
    ...
    echo "Number of dumps per 1tauB is ${dump_freq}, input new value or same value."
    read input
    if ! [[ -z "$input" ]]; then
        dump_freq=$input
    fi
