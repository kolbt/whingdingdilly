...
phi = 0.65
NRange = [100000, 500000, 1000000]
NLines = ['-', ':', '--']
Abox = []
for i in range(0, len(NRange)):
    Abox.append(NRange[i] * np.pi * 0.25 / phi)
Lbox = [ np.sqrt(i) for i in Abox ]
...
phi_leg = []
for i in range(0, len(NRange)):
    phi_leg.append(Line2D([0], [0], lw=1.5, ls=NLines[i], c='k', markeredgecolor='none',
                          label="{:.0f}".format(NRange[i]), markerfacecolor='none',
                          markersize=10))
...
# Set y labels
ax[0].set_ylabel(r'$\gamma \ = \ \left[\Pi^{p}_{bulk} \cdot R_{dense}\right]/L_{box}10^{4}$')
ax[1].set_ylabel(r'$\gamma/L_{box}$')
ax[2].set_ylabel(r'$\gamma / (Pe-Pe_{c}) / L_{box} $')

plt.tight_layout()
plt.savefig('eps_N_analytical_effects.pdf', dpi=500)
plt.close()
