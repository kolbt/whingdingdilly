...
#
#part_num = snapshot.particles.N         # get the total number of particles
...
with hoomd.open(name=myfile, mode='rb') as t:           # open for reading
    ...
    for i in range(0,dumps):
        ...
        type_array[i] = snap.particles.types
        position_array[i] = snap.particles.position     # store particle positions

print(type_array[1][1])
...
for j in range(0, dumps):
    ...
    for k in range(0, len(size_clusters[j])):
        ...
        if size_clusters[j][k] > 25 and size_clusters[j][k] < partnum:
            tot_size[j] += size_clusters[j][k]
            tot_num[j] += 1

    if tot_num[j] > 0:
        MCS[j] = float(tot_size[j]/tot_num[j])/float(partnum)
        GF[j] = float(partnum - tot_size[j]) / float(partnum)
